...
/* QNA 답변 */
.waiting {background:#0489B1;
	color:#fff;
	padding:5px 8px;
}
.waiting1 {background:#F78181;
	color:#fff;
	padding:5px 8px;
}
    </style>
...
<script type="text/javascript">
	...
	function goListPage(pageNumber){
		qna.t_nowPage.value = pageNumber;
		qna.method="post";
		qna.action="Qna";
		qna.submit();
	}
        ...
        <div id="b_right">
                ...
                <tbody>
                    ...
                    <c:forEach items="${list}" var="dto">
                        <tr>
							...
							<c:choose>
    							<c:when test="${empty dto.getAnswer()}">
							        <td><span class="waiting">답변대기</span></td>
							    </c:when>
							    <c:otherwise>
							        <td><span class="waiting1">답변완료</span></td>
							    </c:otherwise>
							</c:choose>
                            <td>${dto.getName() }</td>
            ...
            <div class="paging">
                ...
                <c:if test="${sessionLevel ne 'top' }">
                    <a href="javascript:goPage('Qna','write')" class="write">글쓰기</a>
                </c:if>
            </div>
        </div>
    </div>   
